In Phase 1 I wrote a while True loop manually — every step coded by hand, every decision an if/else inside the loop. It worked, but it was fragile, hard to debug, and impossible to pause mid-execution. Phase 2 replaces that loop with a StateGraph — a directed graph where nodes are functions and edges are decisions. The agent node calls Claude, the routing function checks if tools are needed, the tool node runs them, and an unconditional edge loops back to the agent. Same 7 steps — different architecture. What changed is what you get for free: LangSmith traced every node automatically without a single print statement, the MemorySaver checkpointer saved state after every node so the graph can resume from any point if it crashes, and the conditional edge replaced your manual stop_reason check with an explicit routing function that anyone reading the code can understand immediately. The LangSmith trace you just saw — input, tool call, tool result, output — is your audit trail, your debug tool, and your proof of work all in one. In production this trace is how you find bugs, prove correctness to clients, and monitor agent behaviour at scale. That's the shift from Phase 1 to Phase 2 — from a loop you manage manually to a system that manages itself and shows you everything it's doing.
...
## What LangGraph gives you that Phase 1 didn't
